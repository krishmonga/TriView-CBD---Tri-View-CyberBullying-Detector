#!/usr/bin/env bash
# Download Springer Nature LaTeX journal template (sn-jnl.cls bundle)
# Manual fallback: https://www.springernature.com/gp/authors/campaigns/latex-author-support
set -euo pipefail
DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$DIR"

URL="https://resource-cms.springernature.com/springer-cms/rest/v1/content/19238648/data/v12"
ZIP="springer-sn-jnl-template.zip"

echo "Downloading Springer Nature LaTeX template..."
if curl -fsSL -o "$ZIP" "$URL" && file "$ZIP" | grep -q Zip; then
  unzip -o "$ZIP" -d springer-template-extract
  find springer-template-extract -name 'sn-jnl.cls' -exec cp {} . \;
  find springer-template-extract -name 'sn-mathphys-num.bst' -exec cp {} . \;
  find springer-template-extract -name 'sn-basic.bst' -exec cp {} . \;
  echo "Copied sn-jnl.cls and .bst files to $DIR"
else
  echo "Automatic download failed."
  echo "Please download manually from:"
  echo "  https://www.springernature.com/gp/authors/campaigns/latex-author-support"
  echo "Extract sn-jnl.cls and sn-mathphys-num.bst into:"
  echo "  $DIR"
  exit 1
fi
